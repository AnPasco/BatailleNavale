...
public class PlacerBatStandard {
	...
	public PlacerBatStandard(Partie partie) {
		this.partie = partie;
		...
		// TITRE PRINCIPAL
		this.jLabelTitre = new JLabel(this.nomJoueur + " place tes bateaux !");
		...
		this.listeBateaux.add(new Bateaux("Porte Avion", 5, partie));
		this.listeBateaux.add(new Bateaux("Croiseur", 4, partie));
		this.listeBateaux.add(new Bateaux("Destroyeur", 3, partie));
		this.listeBateaux.add(new Bateaux("Sous-Marin", 3, partie));
		this.listeBateaux.add(new Bateaux("Torpilleur", 3, partie));
	...
	public void fermerFenetre(JFrame maFenetre) {
		if (partie.isModeSolo()) {
			...
			if(partie.isToutBateauPlace()) {
				...
				if(this.partie.isaJ1DeJouer()) {
					this.partie.setGame1(new GameStandard(partie));
				}else {
					this.partie.setGame2(new GameStandard(partie));
				}
				
			}else {
				new PlacerBatStandard(partie);
			}
		}
